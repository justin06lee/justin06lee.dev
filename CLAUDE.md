...
- **The wall hangs inside a window, not down the page.** Left to itself the wall grows a page taller with every project, until the CRT and the store below it are never reached. `Pane` (chrome) bounds it at `min(85vh, 960px)` with its own scrollbar, so the panels stay one screen tall however many projects there are and the hangs below stay one scroll away. The pane fades only the edge the content actually continues past, chains to the page at its ends (`overscroll-behavior: contain` would trap the reader), and is a tab stop only while it overflows; it carries the accessible name, so `MangaPages` inside it doesn't repeat it. An earlier pass hung the pages as a horizontal marquee band instead — the same problem solved by drifting rather than bounding — and the behaviours that needed (inert repeats, focus pause, a reduced-motion scroller) live on in chrome's `marquee`.
